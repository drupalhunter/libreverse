...
#include "Entry_Point_Detector_Elf32_Algorithm.h"
...
namespace libreverse { namespace component {
	...
	{
	  return Entry_Point_Detector_Algorithm::ptr_t ( new Entry_Point_Detector_Elf32_Algorithm() );
	}
      else if ( file_type.compare ( elf_module::Elf_Meta_Info::FILE_TYPE_64BIT ) == 0 )
	{
	  return Entry_Point_Detector_Algorithm::ptr_t ( new Entry_Point_Detector_Elf64_Algorithm() );
	}
      else if ( file_type.compare ( java_module::Java_Meta_Info::FILE_TYPE ) == 0 )	
	{
	  return Entry_Point_Detector_Algorithm::ptr_t ( new Entry_Point_Detector_Java_Algorithm() );
	}
      else if ( file_type.compare ( wpef_module::PE_Meta_Info::PE_FILE_TYPE_32BIT ) == 0 )
	{
	  return Entry_Point_Detector_Algorithm::ptr_t ( new Entry_Point_Detector_WPEF32_Algorithm() );
	}
      else if ( file_type.compare ( wpef_module::PE_Meta_Info::PE_FILE_TYPE_64BIT ) == 0 )
	{
	  return Entry_Point_Detector_Algorithm::ptr_t ( new Entry_Point_Detector_WPEF64_Algorithm() );
	}
      else
	{
