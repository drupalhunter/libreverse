    ...
    Tevis_Zero_Filled_Checker_Algorithm_Factory::get_Algorithm ( std::string name )
    {
        Trace::write_Trace
            ( TraceArea::COMPONENTS,
              TraceLevel::DETAIL,
              "Entering Tevis_Zero_Filled_Checker_Algorithm_Factory::get_Algorithm" );
        ...
        if ( name.compare ( "windows:pe_executable" ) == 0 )
            {
                Trace::write_Trace
                    ( TraceArea::COMPONENTS,
                      TraceLevel::DETAIL,
                      "Exiting Tevis_Zero_Filled_Checker_Algorithm_Factory::get_Algorithm" );

                return Tevis_Zero_Filled_Checker_Algorithm::ptr_t ( new Tevis_Zero_Filled_Checker_WPEF_Algorithm() );
            }
                  ...
                  ( name.compare ( "elf:dynamically_linked_library" ) == 0 ) )
            {
                ...
                return Tevis_Zero_Filled_Checker_Algorithm::ptr_t ( new Tevis_Zero_Filled_Checker_ELF_Algorithm() );
            }
        else
            {
