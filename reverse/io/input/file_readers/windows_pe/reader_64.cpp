...
namespace libreverse { namespace wpef_module {
	...
	m_convert ( false ),
	m_data ( new Header_64() )
    {}

    Reader_64::~Reader_64 (void){}

    void Reader_64::read_DOS_Header (void)
    {
      if ( ! m_file->init() )
	{
    ...
    void Reader_64::read_PE_Header (void)
    {
      read_DOS_Header ();

      wpef_types::DOS_Header::ptr_t dos_hdr = m_data->get_DOS_Header();

      wpef_types::PE_Header_64::ptr_t pe_hdr = m_data->get_PE_Header();

      if ( dos_hdr->has_PE_Header() && (!pe_hdr) )
	{
	  m_file->seek ( dos_hdr->get_PE_Offset() );

	  wpef_types::PE_Header_64::ptr_t new_pe_hdr ( new PE_Header_64() );
    ...
    void Reader_64::read_Section_Headers (void)
    {
      ...
      if ( m_data->get_PE_Section_Header_Count () == 0 )
	{
	  ...
	  wpef_types::PE_Header_64::ptr_t file_hdr = m_data->get_PE_Header();
      ...
      if (dos_hdr_ptr->has_PE_Header())
	{
	  ...
	  wpef_types::PE_Header_64::ptr_t pe_hdr_ptr = m_data->get_PE_Header();
    ...
    std::string
    Reader_64::get_Section_Offset ( std::string name )
    {
      read_PE_Header();

      wpef_types::PE_Header_64::ptr_t pe_hdr_ptr = m_data->get_PE_Header();

      wpef_types::PE_Optional_Header_64::ptr_t opt_hdr_ptr = pe_hdr_ptr->get_Optional_Header();
    ...
    std::string
    Reader_64::get_Section_Length ( std::string name )
    {
      read_PE_Header();
    ...
    bool
    Reader_64::support_File_Type ( void )
    {
      if ( ! m_file->init() )
	{
	  std::cout << "Error initializing file. Checking standard output for error message."
    ...
    std::string
    Reader_64::get_Entry_Point (void)
    {
      read_PE_Header();
    ...
    std::string
    Reader_64::get_Base_Address (void)
    {
      read_PE_Header();
    ...
    void
    Reader_64::preload_Image ( void )
    {
      Trace::write_Trace ( TraceArea::IO,
			   TraceLevel::DETAIL,
			   "Entering Reader_64::preload_Image" );
      ...
      if ( ! this->m_loaded_file )
	{
	  ...
	  */
	  for ( wpef_types::Header_64::Section_Header_Map_t::const_iterator cpos = m_data->get_PE_Section_Header_List_Begin();
    ...
    std::string
    Reader_64::get_Section_String ( std::string name ) const
    {
      return name;
    }

    std::string
    Reader_64::to_String ( void )
    {
      this->read_Headers();
    ...
    Reader_64::get_Text_Strings ( void )
    {
      Trace::write_Trace ( TraceArea::IO,
			   TraceLevel::DETAIL,
      ...
      wpef_types::PE_Header_64::const_ptr_t pe_hdr_ptr = m_data->get_PE_Header();
      ...
      for ( wpef_types::Header_64::Section_Header_Map_t::const_iterator pos = m_data->get_PE_Section_Header_List_Begin();
    ...
    io_types::Text_Data::data_type
    Reader_64::get_UTF16_Strings ( void )
    {
      Trace::write_Trace ( TraceArea::IO,
			   TraceLevel::DETAIL,
	   ...
	   ( ! m_data->get_Export_Directory() ) )
	{
	  ...
	  //-----------------------------
	  boost::uint32_t ordinal_offset = export_dir_ptr->get_Ordinal_Table_RVA () +
	    opt_hdr_ptr->get_Image_Base();
	   ...
	   ( m_data->get_PE_Import_Directory_Count() == 0 ) )
	{
	  ...
	  do
	    {
	      ...
	      m_loaded_file->read_PE_Import_Directory
		( import_dir_ptr );
	  ...
	  for ( wpef_types::Header_64::Import_Directory_List_t::iterator pos =
    ...
    void
    Reader_64::read_Import_Lookup_Table ( wpef_types::PE_Import_Directory::ptr_t import_dir_ptr )
    {
      Trace::write_Trace ( TraceArea::IO,
			   TraceLevel::DETAIL,
			   "Entering Reader_64::read_Import_Lookup_Table" );
      ...
      do
	{
	  ...
	  if ( is_bit_31_set )
		...
		m_loaded_file->get_Present_Position_Value();
      ...
      Trace::write_Trace ( TraceArea::IO,
			   TraceLevel::DETAIL,
			   "Exiting Reader_64::read_Import_Lookup_Table" );
    }

    void
    Reader_64::read_Resource_Table ()
    {
      wpef_types::PE_Header_64::ptr_t pe_hdr_ptr = m_data->get_PE_Header();
      wpef_types::PE_Optional_Header_64::ptr_t opt_hdr_ptr = pe_hdr_ptr->get_Optional_Header();
    ...
    void
    Reader_64::handle_Resource_Directory_Entries ( wpef_types::PE_Resource_Directory::ptr_t parent_ptr,
	   ...
	   PE_Resource_Directory_Entry::NAME )
	{
	  ...
	  wpef_types::PE_Resource_Directory_String::ptr_t new_string_ptr
	    ( new PE_Resource_Directory_String () );

	  m_loaded_file->read_PE_Resource_Directory_String
	    ( new_string_ptr );
      ...
      else
	{
	  ...
	  wpef_types::PE_Resource_Data_Entry::ptr_t new_res_data_ptr
    ...
    void
    Reader_64::read_Load_Config_Table ()
    {
      wpef_types::PE_Header_64::ptr_t pe_hdr_ptr = m_data->get_PE_Header();
      wpef_types::PE_Optional_Header_64::ptr_t opt_hdr_ptr = pe_hdr_ptr->get_Optional_Header();
    ...
    wpef_types::Header_64::ptr_t
    Reader_64::get_Header (void)
    {
      return m_data;
    }

    wpef_types::PE_File::ptr_t
    Reader_64::get_File (void) const
    {
      return m_file;
    }

    void
    Reader_64::read_Exception_Table ()
    {
      wpef_types::PE_Header_64::ptr_t pe_hdr_ptr = m_data->get_PE_Header();
      wpef_types::PE_Optional_Header_64::ptr_t opt_hdr_ptr = pe_hdr_ptr->get_Optional_Header();
	  ...
	  for ( boost::uint32_t index = 0; index < entry_count; ++index )
	    {
	      ...
	      wpef_types::PE_Exception_Table_Entry::ptr_t except_ptr = this->create_Exception_Table_Entry ( machine_number );
    ...
    Reader_64::create_Exception_Table_Entry ( boost::uint16_t type )
    {
      Trace::write_Trace ( TraceArea::IO,
			   TraceLevel::DETAIL,
			   "Entering Reader_64::create_Exception_Table_Entry" );
	...
	case PE_Machine_Types::IMAGE_FILE_MACHINE_AMD64:
	  {
	    ...
	    Trace::write_Trace ( TraceArea::IO,
				 TraceLevel::DETAIL,
				 "Exiting Reader_64::create_Exception_Table_Entry" );
	...
	case PE_Machine_Types::IMAGE_FILE_MACHINE_SH4:
	  {
	    wpef_types::PE_Exception_Table_Entry::ptr_t output ( new ARM_POWERPC_SH_Exception_Table_Entry_64() );
