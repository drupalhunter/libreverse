...
namespace libreverse
{
    namespace mach_module
    {
        ...
        Routines_Command::Routines_Command ( boost::uint32_t cmd, boost::uint32_t cmdsize )
                : m_cmd ( cmd ),
                m_cmdsize ( cmdsize ),
                m_init_address ( 0 ),
                m_init_module ( 0 ),
        ...
        void
        Routines_Command::convert ( bool need_convert )
        {
            if ( need_convert )
            {
        ...
        std::string
        Routines_Command::to_String ( void )
        {
            std::stringstream output;

            output << "Routines Command (32-bit):" << std::endl
