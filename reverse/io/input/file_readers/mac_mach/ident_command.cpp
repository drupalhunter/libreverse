...
namespace libreverse
{
    namespace mach_module
    {
        ...
        Ident_Command::Ident_Command ( boost::uint32_t cmd, boost::uint32_t cmdsize )
                : m_cmd ( cmd ),
                m_cmdsize ( cmdsize )
        {}

        void
        Ident_Command::convert ( bool need_convert )
        {
            if ( need_convert )
            {
        ...
        std::string
        Ident_Command::to_String ( void )
        {
            std::stringstream output;

            output << "Ident Command:" << std::endl
